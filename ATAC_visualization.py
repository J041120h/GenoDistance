...
def ATAC_visualization(adata, pca_type='expression', figsize=(10, 8), 
    ...
    """
    Visualize samples using first and second principal components from PCA analysis.
    ...
    pca_type : str, default 'expression'
        Type of PCA to visualize ('expression' or 'proportion')
    ...
    # Determine which PCA results to use
    if pca_type == 'expression':
        pca_key = 'X_pca_expression'
        group_key = 'X_pca_expression_groups'
        default_title = 'PCA - Expression Data'
    elif pca_type == 'proportion':
        pca_key = 'X_pca_proportion'
    ...
    if pca_key not in adata.uns:
        raise KeyError(f"PCA results not found in adata.uns['{pca_key}']. "
                      f"Please run PCA analysis first.")
    ...
    if pca_df.shape[1] < 2:
        raise ValueError("Need at least 2 principal components for visualization")
    
    # Get first and second PCs
    pc1 = pca_df.iloc[:, 0]  # First PC
    pc2 = pca_df.iloc[:, 1]  # Second PC
    sample_names = pca_df.index
    ...
    plt.scatter(pc1, pc2, s=point_size, alpha=alpha, 
               c='skyblue', edgecolors='black', linewidth=0.5)
    
    # Add sample labels
    for i, sample in enumerate(sample_names):
        plt.annotate(sample, (pc1.iloc[i], pc2.iloc[i]), 
                    xytext=(5, 5), textcoords='offset points',
                    fontsize=8, alpha=0.8)
    
    # Set labels and title
    plt.xlabel(f'PC1 ({pca_df.columns[0]})')
    plt.ylabel(f'PC2 ({pca_df.columns[1]})')
    ...
    if output_dir:
        ...
        sample_filename = f"{pca_type}_pca_samples.png"
    ...
    if grouping_columns:
        ...
        for grouping_col in grouping_columns:
            try:
                ...
                plt.legend(title=f'Groups ({grouping_col})', bbox_to_anchor=(1.05, 1), loc='upper left')
                plt.grid(True, alpha=0.3)
                plt.tight_layout()
                
                # Save group-colored plot
                if output_dir:
                    group_filename = f"{pca_type}_pca_grouped_by_{grouping_col}.png"
...
def ATAC_visualization_both(adata, figsize=(10, 8), point_size=50, 
    ...
    """
    Generate separate plots for both expression and proportion PCA.
    ...
    if 'X_pca_expression' in adata.uns:
        print("Generating Expression PCA plot...")
        ...
        print("Expression PCA results not found in adata.uns")
