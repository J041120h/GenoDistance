...
Clean implementation matching the GPU version's architecture,
but using scanpy's CPU functions.
"""
...
def _extract_sample_metadata(
    ...
    if sample_metadata:
        sample_level_adata.obs = sample_level_adata.obs.join(
            pd.DataFrame(sample_metadata), how="left"
        )
...
def _attempt_combat_correction(
    ...
    """Attempt ComBat correction with timeout and optional covariate preservation."""
...
def _attempt_limma_correction(
    ...
    """Attempt limma-style regression correction (removes batch, preserves covariates)."""
    try:
...
def process_celltype_layer(
    ...
    """Process a single cell-type layer."""
    ...
    if should_normalize:
        if is_atac_data:
            ...
            sc.pp.normalize_total(temp_adata, target_sum=normalization_target_sum)
    ...
    sc.pp.highly_variable_genes(temp_adata, n_top_genes=num_hvgs_to_select, subset=False)
...
def compute_cell_type_proportions(
    ...
    """Compute cell type proportions per sample using crosstab."""
    cell_counts = pd.crosstab(adata.obs[celltype_column], adata.obs[sample_column])
    cell_counts = cell_counts.reindex(index=cell_type_names, columns=sample_names, fill_value=0)
    
    total_cells_per_sample = cell_counts.sum(axis=0)
    total_cells_per_sample[total_cells_per_sample == 0] = 1
    
    return (cell_counts / total_cells_per_sample).astype(float)
...
def compute_pseudobulk_cpu(
    ...
    if verbose:
        ...
        if columns_to_preserve:
            print(f"[Pseudobulk] Preserve cols: {columns_to_preserve}")
    ...
    sc.pp.highly_variable_genes(concatenated_adata, n_top_genes=min(n_features, concatenated_adata.n_vars), subset=True)
    ...
    expression_dataframe.to_csv(os.path.join(pseudobulk_output_dir, "expression_hvg.csv"))
    cell_type_proportions.to_csv(os.path.join(pseudobulk_output_dir, "proportion.csv"))
    
    if verbose:
        print(f"[Pseudobulk] Complete ({time.time() - start_time:.1f}s)")
...
def compute_pseudobulk_adata(
    ...
    expression_dataframe, cell_type_proportions, final_adata = compute_pseudobulk_cpu(
        adata=adata,
        batch_col=batch_col,
...
compute_pseudobulk_layers = compute_pseudobulk_cpu