#!/bin/bash
#SBATCH --job-name=abl_covid_noloo
#SBATCH --output=/dcs07/hongkai/data/harry/result/ablation_noloo/covid/logs/covid_%a.out
#SBATCH --error=/dcs07/hongkai/data/harry/result/ablation_noloo/covid/logs/covid_%a.err
...
echo "[$(date)] no_loo covid size=$SIZE on $(hostname)"
$PY /users/hjiang/GenoDistance/code/claude/ablation/run_ablation_covid.py \
    ...
    --outroot "$R/ablation_noloo/covid" \
    --variants no_loo
