...
def calculate_sample_distances_cell_expression_Test(
    adata: AnnData,
    adataPCA: AnnData,
    output_dir: str,
    ...
    cell_type_centroids = {}
    for cell_type in cell_types:
        indices = adataPCA.obs[adataPCA.obs[cell_type_column] == cell_type].index
        # Get PCA coordinates
        if 'X_pca' in adataPCA.obsm:
            coords = adataPCA.obsm['X_pca'][adataPCA.obs_names.isin(indices)]
        else:
            raise ValueError("PCA coordinates not found in adataPCA.obsm['X_pca']")
