...
def cell_types(
    ...
    if cell_type_column in adata.obs.columns and existing_cell_types:
        ...
        if _recursion_depth == 0:
            ...
            if is_atac:
                sc.pp.neighbors(adata, use_rep=cell_embedding_column, metric="cosine", random_state=42)
            else:
                sc.pp.neighbors(adata, use_rep=cell_embedding_column, n_pcs=cell_embedding_num_PCs, random_state=42)
        ...
        if n_target_clusters is not None:
            ...
            sc.tl.leiden(adata, resolution=leiden_cluster_resolution, flavor="igraph", directed=False, key_added="cell_type", random_state=42)
...
def cell_type_dendrogram(adata, n_clusters, groupby="cell_type", cell_embedding_column="X_pca_harmony", cell_embedding_num_PCs=20, is_atac=False):
    ...
    if not is_atac and cell_embedding_num_PCs is not None and cell_embedding_num_PCs < embedding_data.shape[1]:
        embedding_data = embedding_data[:, :cell_embedding_num_PCs]
